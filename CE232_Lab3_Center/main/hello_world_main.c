...
uint8_t Esp_Node_Add[6] = {48, 174, 164, 253, 234, 158};
// scan parameters
static esp_ble_scan_params_t ble_scan_params = {
    ...
    .scan_interval = 0x50,
    .scan_window = 0x30};
bool arrayCheck(uint8_t arr1[],uint8_t arr2[])
{
    bool Flag = true;
    for (int i=0;i<6;i++)
    {
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT:
        ...
        if (param->scan_rst.search_evt == ESP_GAP_SEARCH_INQ_RES_EVT)
        {
            ...
            if (!alreadyDiscovered(param->scan_rst.bda))
            {
                ...
                // for (int i = 0; i < ESP_BD_ADDR_LEN; i++)
                // {
                //     printf("%u", param->scan_rst.bda[i]);
                //     if (i != ESP_BD_ADDR_LEN - 1)
                //         printf(":");
                // }
                bool t = arrayCheck(Esp_Node_Add,param->scan_rst.bda);
                //printf(t ? "true" : "false");
                ...
                adv_name = esp_ble_resolve_adv_data(param->scan_rst.ble_adv, ESP_BLE_AD_MANUFACTURER_SPECIFIC_TYPE, &adv_name_len);
                if (t == true)
                {
                    printf("\n0xFF=");
                    for (int i = 0; i < 8; i++)
